...
BIN="${ROOT}/bin/image-reader-mcp"
RUST_MAIN="${ROOT}/crates/image-reader-mcp-server/src/main.rs"
...
[[ -f "${BIN}" ]] || report_violation "missing bin/image-reader-mcp"
...
[[ -f "${RUST_MAIN}" ]] || report_violation "missing crates/image-reader-mcp-server/src/main.rs"
...
if [[ -f "${BIN}" ]]; then
  grep -q 'resolve_rust_bin' "${BIN}" || report_violation "bin/image-reader-mcp must resolve Rust rmcp server via resolve_rust_bin"
  grep -q 'printf.*stdio' "${BIN}" || report_violation "bin/image-reader-mcp must default transport to stdio"
  if grep -qE 'use_ts_transport|exec node|IMAGE_READER_MCP_TRANSPORT:-}" == "ts"' "${BIN}"; then
    report_violation "bin/image-reader-mcp must not launch node or retain TS stdio opt-in"
  fi
fi
...
if [[ "${violations}" -gt 0 ]]; then
  ...
  echo "Authority: crates/image-reader-mcp-server/src/main.rs via bin/image-reader-mcp (default stdio)."
  exit 1
fi
